...
void JPSPathFinder::JPSGraphSearch(Eigen::Vector3d start_pt, Eigen::Vector3d end_pt)
{
    ...
    //STEP 1: finish AstarPathFinder::getHeu (from parent), which is the heuristic function
    startPtr -> gScore  = 0;
    ...
    */
        ...
        */
        ...
        if( currentPtr->index == goalIdx_ )
        {
            ...
            RCLCPP_WARN(rclcpp::get_logger("logger"), 
                        "[JPS]{success} Time in JPS is %f ms, path cost is %f m", (time_2 - time_1).seconds()*1000.0, 
    ...
    if((time_2 - time_1).seconds() > 0.1)
        RCLCPP_WARN(rclcpp::get_logger("logger"),
                    "Time consumed in JPS path finding is %f ms",
